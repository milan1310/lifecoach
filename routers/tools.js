...
router.get("/wol/:uuid", async (req, res) => {
  ...
  const foundUser = await Tool.findOne({ uuid });
  res.render("tools/wol", { pageTitle: "Wheel Of Life", uuid: uuid });
});
...
router.post("/wol", (req, res) => {
  ...
  Tool.findOne({ uuid: uuid }, async (err, foundUser) => {
    if (foundUser.wheelOfLife.length === 0) {
      try {
        foundUser.wheelOfLife.push(WOL);
        foundUser.wheelOfLifeRates.push(WOLRates);
        await foundUser.save();
        res.render("tools/wol-result", {
          pageTitle: "Result of Wheel of life",
          uuid: uuid,
          msg: "Congratulations you have succefully completed the Wheel of Life",
        });
      } catch (error) {
...
router.post("/wol", (req, res) => {
  ...
  Tool.findOne({ uuid: uuid }, async (err, foundUser) => {
    if (foundUser.wheelOfLife.length === 0) {
      try {
        const WOL = Object.entries(req.body);
        foundUser.wheelOfLife.push(WOL);
        await foundUser.save();
        res.render("tools/wol-result", {
          pageTitle: "Result of Wheel of life",
...
router.get("/wol-data", async (req, res) => {
  ...
  await Tool.findOne({ uuid: uuid }, async (err, foundUser) => {
    if (err) {
      return res.send(err);
    } else {
      res.json({
...
router.get("/baseline/:uuid", (req, res) => {
  const uuid = req.params.uuid;
  res.render("tools/baseline", { pageTitle: "Life Mastery Circle", uuid: uuid });
});
    ...
    if (foundUser.baselines.length === 0) {
      try {
        ...
        res.json({
          status: "success",
          user: foundUser,
          message: "You have success fully completed your Life Mastery Circle.",
        });
      } catch (error) {
        res.json({
          status: "fail",
      ...
      res.json({
        status: "alreadyDone",
        user: foundUser,
        message: "You have already done your Life Mastery Circle here is your result.",
      });
    }
  });
});
...
router.get("/necodes/:uuid", (req, res) => {
  const uuid = req.params.uuid;
  res.render("tools/NEcodes", { pageTitle: "N Patterns/E Patterns", uuid: uuid });
});
...
// Decision Making Reckoner
router.get("/decisionmakingwheel/:uuid", (req, res) => {
  const uuid = req.params.uuid;
  res.render("tools/decision-making-wheel", {
  ...
  Tool.findOne({ uuid: uuid }, async (err, foundUser) => {
    if (foundUser.decisionMakingWheel.length === 0) {
      try {
        ...
        res.render("tools/decision-wheel-result", {
...
router.get("/defineoutcome/:uuid", (req, res) => {
  const uuid = req.params.uuid;
  res.render("tools/define-my-outcome", {
    ...
    Tool.findOne({ uuid: uuid }, async (err, foundUser) => {
      if (foundUser.outcomes.length === 0) {
        try {
          ...
          res.render("tools/outcomes-result", {
